...
class EmployeeApi {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (employee.name.length < minEmployeeTitleLength) {
          reject(`Name must be at least ${minEmployeeTitleLength} characters.`);
        }
  ...
  static saveSkills(skills, rating, id) {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(getUpdatedSkills(rating, skills, id));
      }, delay);
    });
  }

  static updateRatingValue(rating, topic, template) {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({[template]: {[topic]: rating}});
      }, delay);
    });
  }
